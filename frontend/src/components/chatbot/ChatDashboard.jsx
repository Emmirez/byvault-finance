...
import AdminAlertBell from "../../pages/admin/Notification/AdminAlertBell";
  ...
  return (
    <nav className="hidden lg:flex flex-col items-center w-16 xl:w-52 h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-shrink-0 py-4 gap-1 sticky top-0">
      ...
      {NAV_ITEMS.map(({ label, icon: Icon, to }) => {
        ...
        const badge =
          label === "Live Chat" && unreadChats > 0 ? unreadChats : null;
        ...
        return (
          ...
          >
            ...
            <span className="hidden xl:block ml-3 text-sm font-medium">
              {label}
            </span>
...
const ChatDashboard = () => {
  ...
  const [selectedSession, setSelectedSession] = useState(() => {
    try {
      const s = localStorage.getItem(SELECTED_SESSION_KEY);
  ...
  useEffect(() => {
    try {
      ...
      if (s) {
        ...
        Object.entries(p).forEach(([id, msgs]) =>
          messagesMap.current.set(id, msgs),
        );
      }
    } catch (e) {
      console.error(e);
  ...
  useEffect(() => {
    ...
    try {
      const obj = {};
      messagesMap.current.forEach((v, k) => {
        obj[k] = v;
      });
  ...
  useEffect(() => {
    ...
    socket.on("new-session", (session) => {
      ...
      showBrowserNotification(
        "New Chat Session",
        `${session.userName} started a chat`,
      );
    });
    ...
    socket.on("new-message", (message) => {
        ...
        s.map((sess) =>
          sess._id === message.sessionId
            ? {
                ...
                unreadCount: (sess.unreadCount || 0) + 1,
              }
            : sess,
        ),
      );
      ...
      if (!message.isAdmin && current?._id !== message.sessionId) {
        ...
        showBrowserNotification(
          "New Message",
          `${message.userName || "User"}: ${message.message.substring(0, 50)}${message.message.length > 50 ? "..." : ""}`,
        );
      }
      if (current && message.sessionId === current._id) {
        setMessages((msgs) => {
          const exists = msgs.some(
            (m) =>
              m._id === message._id ||
              (m._optimistic && m.message === message.message),
          );
          if (exists) return msgs;
          const idx = msgs.findIndex(
            (m) => m._optimistic && m.message === message.message,
          );
          if (idx !== -1) {
            const next = [...msgs];
            next[idx] = { ...message, _optimistic: false };
            return next;
          }
          return [...msgs, message];
        });
        setUnreadSessions((s) => {
          const n = new Set(s);
          n.delete(message.sessionId);
          return n;
        });
        setSessions((s) =>
          s.map((sess) =>
            sess._id === message.sessionId ? { ...sess, unreadCount: 0 } : sess,
          ),
        );
      }
    });
    ...
    socket.on("session-ended", (sessionId) => {
      ...
      if (selectedSessionRef.current?._id === sessionId) {
    ...
    socket.on("user-online", ({ sessionId, socketId }) =>
      setSessions((s) =>
        s.map((sess) =>
          sess._id === sessionId ? { ...sess, socketId, isOnline: true } : sess,
        ),
      ),
    );
    socket.on("user-offline", ({ sessionId }) =>
      setSessions((s) =>
        s.map((sess) =>
          sess._id === sessionId
            ? { ...sess, socketId: null, isOnline: false }
            : sess,
        ),
      ),
    );
    socket.on("user-active", ({ sessionId }) =>
      setSessions((s) =>
        s.map((sess) =>
          sess._id === sessionId
            ? { ...sess, lastActivity: new Date(), isOnline: true }
            : sess,
        ),
      ),
    );
    socket.on("message-sent", (data) => {
      if (data.sessionId === selectedSessionRef.current?._id)
        setMessages((msgs) =>
          msgs.map((m) =>
            m._optimistic && m.message === data.message
  ...
  useEffect(() => {
    if (selectedSession) {
      ...
      setUnreadSessions((s) => {
        const n = new Set(s);
        n.delete(selectedSession._id);
        return n;
      });
      setSessions((s) =>
        s.map((sess) =>
          sess._id === selectedSession._id ? { ...sess, unreadCount: 0 } : sess,
        ),
      );
    ...
    setSessions((prev) =>
      prev.map((s) =>
        s._id === selectedSession._id
          ? { ...s, lastMessage: inputMessage, lastActivity: new Date() }
          : s,
      ),
    );
    setInputMessage("");
  ...
  const confirmDelete = () => {
    ...
    socketRef.current.emit("admin-end-session", {
    ...
    if (selectedSession?._id === deleteTarget._id) {
      setSelectedSession(null);
      setMessages([]);
    }
    ...
    <div className="flex-1 overflow-y-auto" ref={sessionListRef}>
      {sessions.length === 0 ? (
        ...
        sessions.map((session) => {
          const hasUnread =
            unreadSessions.has(session._id) || session.unreadCount > 0;
          return (
            <div
              key={session._id}
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="relative flex-shrink-0">
                    ...
                    <div
                      className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 ${session.socketId ? "bg-green-500" : "bg-gray-300"}`}
                    />
                    {hasUnread && (
                      <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center shadow-md">
                        !
                      </span>
                    )}
                  </div>
              ...
              {selectedSession ? (
                <>
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    <div
                      className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 ${selectedSession.socketId ? "bg-green-500" : "bg-gray-300"}`}
                    />
                  </div>
                  ...
                  messages.map((msg, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`max-w-[75%] sm:max-w-[65%] flex flex-col ${isAdmin ? "items-end" : "items-start"}`}
                        >
                          {isAdmin && (
                          ...
                          <div
                            className={`flex items-center gap-1 mt-1 text-xs text-gray-400 px-1 ${isAdmin ? "justify-end" : "justify-start"}`}
                          >
                            <Clock size={9} />
