...
const saveMessages = (msgs) => {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(msgs.slice(-200))); }
  catch {}
};
const loadSession = () => {
...
const saveSession = (id) => {
  try { if (id) localStorage.setItem(SESSION_KEY, id); }
  catch {}
};
const loadUnread = () => {
...
const saveUnread = (n) => {
  try { localStorage.setItem(UNREAD_KEY, String(n)); }
  catch {}
};
...
const ChatWidget = ({ position = "right", primaryColor = "#2563eb" }) => {
    ...
    if (!hasJoined.current) {
      ...
      setTimeout(() => {
        setMessages((prev) => [
          ...prev,
          {
            message:    "👋 Hello! Thanks for contacting Byvault Support. An agent will be with you shortly.",
            sender:     "system",
            isAdmin:    false,
            createdAt:  new Date().toISOString(),
            _localOnly: true,
  ...
  // ── Chat window ─────────────────────────────────────────────────────────────
  return (
    <div style={widgetStyle} className="w-[380px] font-sans">
      <div
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-semibold text-sm leading-tight">Byvault Support</p>
            ...
            >
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                  ...
                  <div>
                    <p className="font-semibold text-slate-700 text-sm">How can we help?</p>
                    <p className="text-slate-400 text-xs mt-1 leading-relaxed">
                      Send us a message and our team<br />will get back to you shortly.
                    </p>
                  </div>
                </div>
              )}
              ...
              {groupedMessages.map(({ date, messages: dayMsgs }) => (
                <div key={date}>
                  <div className="flex items-center gap-2 my-3">
                    ...
                    <span className="text-[11px] text-slate-400 font-medium px-2">{date}</span>
                    <div className="flex-1 h-px bg-slate-200" />
                  </div>
                  ...
                  {dayMsgs.map((msg) => {
                    ...
                    if (isSystem) {
                      return (
                        <div key={msg._index} className="flex justify-center my-2">
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          style={!msg.isAdmin && isUser ? { backgroundColor: primaryColor } : {}}
                        >
            ...
            <div className="px-3 pb-3 pt-2 bg-white border-t border-slate-100 flex-shrink-0">
              <div className="flex items-end gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 focus-within:border-blue-300 transition-colors">
                ...
                <button
                  ...
                  style={{ backgroundColor: inputMessage.trim() && isConnected ? primaryColor : undefined }}
                ...
                >
                  <Send
                    size={15}
                    className={inputMessage.trim() && isConnected ? "text-white" : "text-slate-400"}
                  />
                </button>
              </div>
